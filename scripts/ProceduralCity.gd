...
func _create_chunk(chunk_coord: Vector2i, key: String, center_chunk: Vector2i) -> void:
	...
	_add_terrain_visual(chunk_node, chunk_coord, zone)
	_add_ride_surface(chunk_node)
...
func _add_ride_surface(parent: Node3D) -> void:
	var ground_body := StaticBody3D.new()
	ground_body.name = "RideSurface"
	var ride_y: float = BikeRigScript.RIDE_SURFACE_Y
	ground_body.position = Vector3(CHUNK_CENTER, ride_y - 0.5, CHUNK_CENTER)
...
func _add_terrain_visual(parent: Node3D, chunk_coord: Vector2i, zone: CityTerrainScript.Zone) -> void:
	...
	for gz in TERRAIN_GRID + 1:
		for gx in TERRAIN_GRID + 1:
			...
			var h := CityTerrainScript.sample_height(wx, wz) - 0.45
...
func _add_road_segment(road_body: StaticBody3D, _chunk_coord: Vector2i, local_pos: Vector3, size: Vector3, mat: Material) -> void:
	var mesh_inst := MeshInstance3D.new()
...
func _pick_npc_spawn(rng: RandomNumberGenerator, chunk_coord: Vector2i) -> Vector3:
	for _attempt in 12:
		...
		var wz := chunk_coord.y * chunk_size + lz
		var hy := CityTerrainScript.sample_height(wx, wz)
		return Vector3(lx, hy, lz)
	return Vector3.INF

func _add_city_blocks(parent: Node3D, chunk_coord: Vector2i, rng: RandomNumberGenerator) -> void:
	var slots := [
		Vector3(22.0, 0.0, 22.0), Vector3(98.0, 0.0, 22.0),
		Vector3(22.0, 0.0, 98.0), Vector3(98.0, 0.0, 98.0),
		Vector3(14.0, 0.0, 60.0), Vector3(106.0, 0.0, 60.0),
		Vector3(60.0, 0.0, 14.0), Vector3(60.0, 0.0, 106.0)
	]
	...
	for slot: Vector3 in slots:
		...
		building.position = Vector3(pos.x, BikeRigScript.RIDE_SURFACE_Y, pos.z)
...
func _add_garage(parent: Node3D, chunk_coord: Vector2i) -> void:
	...
	var door_w := 5.2
	var door_h := 4.2
	var gy := BikeRigScript.RIDE_SURFACE_Y
	_add_garage_box(garage, "ApproachPad", Vector3(0.0, gy + 0.03, 7.2), Vector3(door_w + 2.4, 0.06, 5.0), pad_mat, false)
	...
	_add_garage_box(garage, "BackWall", Vector3(0.0, BikeRigScript.RIDE_SURFACE_Y + height * 0.5, -depth * 0.5 + wall_t * 0.5), Vector3(width, height, wall_t), wall_mat, true)
	_add_garage_box(garage, "LeftWall", Vector3(-width * 0.5 + wall_t * 0.5, gy + height * 0.5, 0.0), Vector3(wall_t, height, depth), wall_mat, true)
	_add_garage_box(garage, "RightWall", Vector3(width * 0.5 - wall_t * 0.5, gy + height * 0.5, 0.0), Vector3(wall_t, height, depth), wall_mat, true)
	var side_span := (width - door_w) * 0.5
	_add_garage_box(garage, "FrontLeftPillar", Vector3(-door_w * 0.5 - side_span * 0.5, gy + height * 0.5, depth * 0.5 - wall_t * 0.5), Vector3(side_span, height, wall_t), wall_mat, true)
	_add_garage_box(garage, "FrontRightPillar", Vector3(door_w * 0.5 + side_span * 0.5, gy + height * 0.5, depth * 0.5 - wall_t * 0.5), Vector3(side_span, height, wall_t), wall_mat, true)
	_add_garage_box(garage, "DoorLintel", Vector3(0.0, gy + height - (height - door_h) * 0.5, depth * 0.5 - wall_t * 0.5), Vector3(door_w, height - door_h, wall_t), wall_mat, true)
	_add_garage_box(garage, "DoorInterior", Vector3(0.0, gy + door_h * 0.5, depth * 0.5 - 1.2), Vector3(door_w - 0.4, door_h - 0.2, 1.6), interior_mat, false)
	_add_garage_box(garage, "DoorFrameL", Vector3(-door_w * 0.5 - 0.12, gy + door_h * 0.5, depth * 0.5 + 0.1), Vector3(0.2, door_h, 0.25), trim_mat, false)
	_add_garage_box(garage, "DoorFrameR", Vector3(door_w * 0.5 + 0.12, gy + door_h * 0.5, depth * 0.5 + 0.1), Vector3(0.2, door_h, 0.25), trim_mat, false)
	_add_garage_box(garage, "DoorRamp", Vector3(0.0, gy + 0.12, depth * 0.5 + 2.0), Vector3(door_w, 0.24, 2.8), trim_mat, false)
	var sign := MeshInstance3D.new()
	sign.name = "GarageSign"
	var sign_mesh := BoxMesh.new()
	sign_mesh.size = Vector3(6.0, 1.4, 0.3)
	sign.mesh = sign_mesh
	sign.position = Vector3(0.0, gy + height + 0.5, depth * 0.5 + 0.2)
	sign.material_override = trim_mat
	garage.add_child(sign)
	var beacon := OmniLight3D.new()
	beacon.name = "GarageBeacon"
	beacon.position = Vector3(0.0, gy + 4.5, depth * 0.5 + 2.5)
	...
	zone.monitoring = true
	zone.position = Vector3(0.0, gy + 2.0, depth * 0.5 + 5.5)
...
func _add_wizard_tower(parent: Node3D, chunk_coord: Vector2i) -> void:
	...
	_add_garage_box(tower, "Foundation", Vector3(0.0, gy + 0.2, 0.0), Vector3(9.0, 0.4, 9.0), stone_dark, true)
	_add_garage_box(tower, "TowerBase", Vector3(0.0, gy + 2.5, 0.0), Vector3(6.0, 5.0, 6.0), stone, true)
	_add_garage_box(tower, "TowerMid", Vector3(0.0, gy + 6.8, 0.0), Vector3(4.8, 4.2, 4.8), stone, true)
	_add_garage_box(tower, "TowerTop", Vector3(0.0, gy + 10.2, 0.0), Vector3(3.6, 3.0, 3.6), stone_dark, true)
	_add_garage_box(tower, "Roof", Vector3(0.0, gy + 12.8, 0.0), Vector3(4.4, 1.6, 4.4), roof, false)
	_add_garage_box(tower, "Spire", Vector3(0.0, gy + 14.6, 0.0), Vector3(0.5, 2.4, 0.5), glow, false)
	_add_garage_box(tower, "DoorGlow", Vector3(0.0, gy + 1.4, 3.15), Vector3(2.2, 2.6, 0.2), glow, false)
	var zone := Area3D.new()
	zone.name = "WizardTowerEntrance"
	zone.monitoring = true
	zone.position = Vector3(0.0, gy + 1.5, 4.2)
